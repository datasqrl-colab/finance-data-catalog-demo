...
def _balance_rows(tables, rng, world, series, txns, holds):
    ...
    for a in world["accounts"]:
        ...
        for day in balance_dates(a):
            ...
            event = (at(day, 23, 30) + dt.timedelta(seconds=rng.randint(0, 1500))
                     if day < AS_OF_DATE else AS_OF - dt.timedelta(minutes=rng.randint(40, 90)))
...
def _accrual_rows(tables, rng, world, series, txns):
    ...
    for a in world["accounts"]:
        ...
        for day in balance_dates(a):
            ...
            event = (at(day, 23, 45) + dt.timedelta(seconds=rng.randint(0, 840))
                     if day < AS_OF_DATE else AS_OF - dt.timedelta(minutes=rng.randint(30, 60)))
            stamp_bronze(rng, row, event)
