...
def stamp_bronze(rng, row, event, max_lag_hours=36):
    """Set source_updated_at shortly after the event and ingested_at 1 minute to max_lag_hours later.

    Events older than the activity window are treated as part of the initial backfill load, which
    was ingested during the first 45 days of the window.
    """
    event = min(event, AS_OF - dt.timedelta(minutes=10))
    updated = min(event + dt.timedelta(seconds=rng.randint(0, 1800)), AS_OF - dt.timedelta(minutes=5))
    base = updated
    if updated < WINDOW_START:
        base = WINDOW_START + dt.timedelta(seconds=rng.randint(0, 45 * 86400))
    ingested = min(base + dt.timedelta(seconds=rng.randint(60, max_lag_hours * 3600)), AS_OF)
