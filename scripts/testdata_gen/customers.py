...
def _address_rows(tables, rng, customers):
    ...
    for c in customers:
        ...
        # Verification lands a couple of days after the address takes effect, and the source record
        # cannot have last changed before the verification it carries.
        address_verified = business_time(rng, min(current_from + dt.timedelta(days=2), AS_OF_DATE))
        row = tables.add(stem, customer_id=c["id"], address_type="PRIMARY", address_line_1=c["street"],
                         ...
                         verified_at=ts(address_verified) if verified else None,
                         source_system=c["source_system"])
        stamp_bronze(rng, row, address_verified if verified else business_time(rng, current_from))
        if c["type"] in ("BUSINESS", "TRUST") or rng.random() < 0.05:
            mailing_verified = business_time(rng, c["start"])
            row = tables.add(stem, customer_id=c["id"], address_type="MAILING",
                             ...
                             verification_status="VERIFIED", verified_at=ts(mailing_verified),
                             source_system=c["source_system"])
            stamp_bronze(rng, row, mailing_verified)
        # A separate billing address is common for businesses and trusts, and occasional for
        # individuals who bill to somewhere other than where they live. Populating it means a
        # consumer filtering on address_type has a real category rather than a single row.
        if c["type"] in ("BUSINESS", "TRUST") or rng.random() < 0.04:
            billing_verified = business_time(rng, min(c["start"] + dt.timedelta(days=rng.randint(0, 45)),
                                                      AS_OF_DATE - dt.timedelta(days=1)))
            billing_city = rng.choice(CITIES)
            row = tables.add(stem, customer_id=c["id"], address_type="BILLING",
                             address_line_1=f"{rng.randint(10, 9899)} {rng.choice(STREETS)} {rng.choice(STREET_KIND)}",
                             address_line_2=f"Suite {rng.randint(100, 899)}" if c["type"] == "BUSINESS" else None,
                             city=billing_city[0], state_province=billing_city[1],
                             postal_code=billing_city[2] + f"{rng.randint(1, 99):02d}", country_code="US",
                             effective_from=ds(c["start"]), effective_to=None, is_current=True,
                             verification_status=weighted(rng, [("VERIFIED", 88), ("UNVERIFIED", 12)]),
                             verified_at=ts(billing_verified), source_system=c["source_system"])
            stamp_bronze(rng, row, billing_verified)


# Share of customers who change a contact detail at some point, and how often the change lands in
# the recent part of the window rather than the long tail before it.
CONTACT_CHANGE_RATE = 0.22
RECENT_CHANGE_SHARE = 0.6
EMAIL_DOMAINS = ["example.com", "example.net", "mail.example.com"]


def _replacement_contact(rng, c, kind, value):
    """A new value for a contact the customer has changed: a ported number or a new mailbox."""
    if kind == "EMAIL":
        handle, _, domain = value.partition("@")
        return f"{handle}.{rng.choice(['new', 'alt', str(rng.randint(2, 89))])}@{rng.choice(EMAIL_DOMAINS)}"
    return f"+1-{c['city'][5]}-555-0{rng.randint(300, 989)}"


def _contact_change_time(rng, c):
    """When the change happened, weighted toward the recent window as the address changes are."""
    floor = c["start"] + dt.timedelta(days=90)
    ceiling = AS_OF_DATE - dt.timedelta(days=2)
    if floor >= ceiling:
        return None
    recent_floor = max(floor, dt.date(2023, 1, 1))
    if rng.random() < RECENT_CHANGE_SHARE and recent_floor < ceiling:
        return business_time(rng, rand_date(rng, recent_floor, ceiling))
    return business_time(rng, rand_date(rng, floor, ceiling))


def _contact_status(rng):
    """Most contacts verify; a few never do, and a few are known bad."""
    return weighted(rng, [("VERIFIED", 91), ("UNVERIFIED", 7), ("INVALID", 2)])


def _contact_rows(tables, rng, customers):
    """The contact stream, carrying supersessions: a changed number or mailbox leaves the old row
    behind with its original timestamps and a newer row alongside it holding the current value."""
    ...
    for c in customers:
        ...
        verified_at = business_time(rng, verified_day)
        ...
        changed_kind = rng.randrange(len(kinds)) if rng.random() < CONTACT_CHANGE_RATE else None
        changed_at = _contact_change_time(rng, c) if changed_kind is not None else None
        if changed_at is None:
            changed_kind = None
        for i, (kind, value) in enumerate(kinds):
            current_value, superseded = value, None
            if i == changed_kind:
                current_value = _replacement_contact(rng, c, kind, value)
                superseded = value
            if superseded is not None:
                # The old row keeps the timestamps it was written with and stops being primary.
                row = tables.add(stem, customer_id=c["id"], contact_type=kind, contact_value=superseded,
                                 is_primary=False, verification_status="VERIFIED", verified_at=ts(verified_at),
                                 marketing_opt_in=rng.random() < 0.45, transactional_opt_in=rng.random() < 0.95,
                                 source_system=c["source_system"])
                stamp_bronze(rng, row, verified_at)
            status = _contact_status(rng)
            seen_at = changed_at if superseded is not None else verified_at
            row = tables.add(stem, customer_id=c["id"], contact_type=kind, contact_value=current_value,
                             is_primary=i == 0, verification_status=status,
                             verified_at=ts(seen_at) if status == "VERIFIED" else None,
                             marketing_opt_in=rng.random() < 0.45, transactional_opt_in=rng.random() < 0.95,
                             source_system=c["source_system"])
            stamp_bronze(rng, row, seen_at)
